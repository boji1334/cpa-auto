...
param(
    ...
    [switch]$NoCaddy,
    [switch]$AskSecrets,
    [switch]$WithManagerPlus,
    [string]$ManagerPlusImage = "seakee/cpa-manager-plus:latest",
    [int]$ManagerPlusPort = 18317,
    [string]$ManagerPlusDomain = "",
    [string]$ManagerPlusAdminKey = "",
    [ValidateSet("auto", "http", "resp", "subscribe")]
    [string]$ManagerPlusCollectorMode = "auto",
    [switch]$NoStart,
    [switch]$Help
)
...
Options:
  ...
  -WithManagerPlus         Install CPA Manager Plus alongside CPA.
  -ManagerPlusDomain D     HTTPS domain for Manager Plus when using Caddy or an external reverse proxy.
  -ManagerPlusPort P       Host port for Manager Plus. Default: 18317.
  -ManagerPlusAdminKey K   Manager Plus login admin key. Default: auto-generated.
  -ManagerPlusImage I      Manager Plus image. Default: seakee/cpa-manager-plus:latest.
...
if ($WithManagerPlus -or -not [string]::IsNullOrWhiteSpace($ManagerPlusDomain)) {
    $WithManagerPlus = $true
}

if ($WithManagerPlus -and ($Mode -eq "local" -or $UseCaddy -or -not [string]::IsNullOrWhiteSpace($ManagerPlusDomain))) {
    $ManagerPlusBindHost = "127.0.0.1"
}
elseif ($WithManagerPlus) {
    $ManagerPlusBindHost = "0.0.0.0"
}
else {
    $ManagerPlusBindHost = "127.0.0.1"
}
...
usage-statistics-enabled: $usageEnabled
redis-usage-queue-retention-seconds: $usageRetention
...
"@
    if ($WithManagerPlus -and -not [string]::IsNullOrWhiteSpace($ManagerPlusDomain)) {
        $caddyfile += @"
...
Commands:
  ...
  manager-health
              Check CPA Manager Plus local health
...
switch ($Command.ToLowerInvariant()) {
    ...
    "manager-logs" {
        Invoke-Compose @("logs", "-f", "cpa-manager-plus")
    }
    "update" {
        Invoke-Compose @("pull")
        Invoke-Compose @("up", "-d")
    ...
    "manager-health" {
        $managerPort = Get-DotEnvValue ".env" "MANAGER_PLUS_PORT"
        if ([string]::IsNullOrWhiteSpace($managerPort)) { $managerPort = "18317" }
        Invoke-RestMethod -Uri "http://127.0.0.1:$managerPort/health"
    }
    "url" {
        if (-not [string]::IsNullOrWhiteSpace($Domain)) {
    ...
    "backup" {
        ...
        $paths = @(".env", ".credentials", "config.yaml", "docker-compose.yml", "docker-compose.caddy.yml", "Caddyfile", "auths", "logs", "manager-plus-data", "caddy_data", "caddy_config") | Where-Object { Test-Path $_ }
...
if ($WithManagerPlus) {
    New-Item -ItemType Directory -Force -Path "manager-plus-data" | Out-Null
}
...
if ($WithManagerPlus -and [string]::IsNullOrWhiteSpace($ManagerPlusAdminKey)) { $ManagerPlusAdminKey = "cmp_admin_$(New-HexSecret 24)" }
...
Set-DotEnvValue ".env" "WITH_MANAGER_PLUS" "$($WithManagerPlus.ToString().ToLowerInvariant())"
Set-DotEnvValue ".env" "MANAGER_PLUS_IMAGE" $ManagerPlusImage
Set-DotEnvValue ".env" "MANAGER_PLUS_BIND_HOST" $ManagerPlusBindHost
Set-DotEnvValue ".env" "MANAGER_PLUS_PORT" "$ManagerPlusPort"
Set-DotEnvValue ".env" "MANAGER_PLUS_DOMAIN" $ManagerPlusDomain
Set-DotEnvValue ".env" "MANAGER_PLUS_ADMIN_KEY" $ManagerPlusAdminKey
Set-DotEnvValue ".env" "MANAGER_PLUS_COLLECTOR_MODE" $ManagerPlusCollectorMode
...
$ManagerPlusUrl = ""
if ($WithManagerPlus) {
    if (-not [string]::IsNullOrWhiteSpace($ManagerPlusDomain)) {
        $ManagerPlusUrl = "https://$ManagerPlusDomain"
    }
    elseif ($Mode -eq "local") {
        $ManagerPlusUrl = "http://localhost:$ManagerPlusPort"
    }
    else {
        $ManagerPlusUrl = "http://SERVER_IP:$ManagerPlusPort"
    }
}
...
if ($WithManagerPlus) {
    $credentials += @(
        "MANAGER_PLUS_URL=$ManagerPlusUrl",
        "MANAGER_PLUS_ADMIN_KEY=$ManagerPlusAdminKey"
    )
}
...
if ($WithManagerPlus) {
    Write-Host "Manager Plus:        $ManagerPlusUrl/management.html"
    Write-Host "Manager admin key:   $ManagerPlusAdminKey"
}
Write-Host ""
